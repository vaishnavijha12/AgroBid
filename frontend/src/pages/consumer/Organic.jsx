...
export default function OrganicPage() {
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] font-sans text-slate-800 pb-12">

      {/* 3D Earthy Background Accents */}
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-20 -left-20 w-[600px] h-[600px] bg-stone-200/40 rounded-full blur-[100px]"></div>
        <div className="absolute top-1/2 right-0 w-[500px] h-[500px] bg-orange-100/40 rounded-full blur-[100px]"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-12">
          <button onClick={() => navigate(-1)} className="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all text-stone-700">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-stone-700 to-amber-800">
            Organic Staples
          </h1>
          <button className="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl text-stone-700">
            <Filter className="w-6 h-6" />
          </button>
        </div>

        {/* Featured Hero Card */}
        <div className="mb-16 bg-gradient-to-r from-amber-700 to-stone-600 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div>
              <span className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-full text-sm font-bold uppercase tracking-wider mb-4 inline-block">100% Natural</span>
              <h2 className="text-4xl md:text-5xl font-black mb-4 leading-tight">Purity <br />You Can Trust</h2>
              <p className="text-stone-200 text-lg mb-8 max-w-md">Certified organic groceries free from chemicals and preservatives. Good for you, good for the planet.</p>
              <button className="bg-white text-stone-800 px-8 py-3 rounded-2xl font-bold hover:shadow-lg hover:scale-105 transition-all">
                Shop Now
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((item) => (
            <div key={item.id} className="bg-white rounded-3xl p-4 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group">
              <div className="relative h-64 rounded-2xl overflow-hidden mb-4 bg-stone-50">
              ...
              <div className="px-2 pb-2">
                ...
                <p className="text-slate-400 text-sm mb-4">Chemical Free</p>

                <div className="flex items-center justify-between">
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-black text-stone-700">₹{item.price}</span>
                    <span className="text-sm font-medium text-slate-400">/ {item.unit}</span>
                  </div>
                  <button
                    onClick={() => addToCart(item)}
                    className="bg-slate-900 text-white p-3 rounded-xl hover:bg-stone-600 transition-colors shadow-lg"
                  >
